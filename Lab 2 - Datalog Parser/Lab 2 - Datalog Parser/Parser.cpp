...
void Parser::fact() {
	...
	domain.insert((tokenList.at(i).getValue()));
...
void Parser::stringList() {
	if (currentToken == COMMA) {
		...
		domain.insert((tokenList.at(i-1).getValue()));
		tempParam = Parameter(tokenList.at(i-1).getValue());
		tempPred.addParam(tempParam);
